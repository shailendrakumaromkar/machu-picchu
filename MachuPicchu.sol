...
import {RUP} from "./RUP.sol";
import {OTPGeneratorAndVerifier} from "./OTPGeneratorAndVerifier.sol";
  ...
  */
    ...
    function getAssessmentsDoneByAWatcher(uint8 monthId)
     A   public
        ...
        returns (uint256[] memory)
    {
        return watcherssessments[monthId][msg.sender];
    }
    ...
    function monthlyLossCalculation() public {
        ...
        uint avgAllGroupLoss = group1Loss + group2Loss + group3Loss / 3;
        ...
        for (uint256 i = 0; i < assessments[watcherAddress[0]].length; i++) {
            W1TotalAssessment += Assessments[assessments[watcherAddress[0]][i]].all;
        }

        for (uint256 i = 0; i < assessments[watcherAddress[1]].length; i++) {
            W2TotalAssessment += Assessments[assessments[watcherAddress[1]][i]].all;
        }
       ...
       //Get initial Stake Amount for each Watcher
         W1StakeAmount = Stakes[watcherAddress[0]][currentMonth];
         W2StakeAmount = Stakes[watcherAddress[1]][currentMonth];
       ...
       //Calculate Final Refund Amount for each Watcher
        ...
        //Transfer the Refund amount to Each Watcher
         tokenContract.transfer(watcherAddress[0],W1RefundAmount);
